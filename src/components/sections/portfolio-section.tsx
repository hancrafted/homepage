...
import { PORTFOLIO_DECKS, type Deck } from '@/content/decks';
    ...
    <CardFooter className="pt-2 p-0 flex flex-col gap-2">
      ...
      {hasVideo && (
        <Button
          ...
          size="sm"
          variant="ghost"
          className="w-full font-mono text-xs bg-black/[0.04] hover:bg-black/[0.08] dark:bg-white/[0.04] dark:hover:bg-white/[0.08] text-foreground/90"
        >
...
function DeckCard({ deck, locale }: { deck: Deck; locale: LocaleCode }) {
  ...
  return (
    <SpotlightCard className="flex flex-col justify-between p-7 rounded-3xl bg-card border border-border/60 dark:border-white/[0.06] dark:bg-white/[0.035] backdrop-blur-md shadow-xl shadow-black/5 dark:shadow-black/20 hover:bg-muted/40 dark:hover:bg-white/[0.06] transition-all group">
      <DeckCardHeader
      ...
      <CardContent className="space-y-4 p-0 py-4 text-xs">
        <div className="rounded-2xl bg-amber-500/10 p-4 space-y-1.5">
          <div className="flex items-center gap-1 font-semibold text-amber-500 dark:text-amber-400 text-[11px] uppercase tracking-wider font-mono">
        ...
        <div className="flex flex-wrap gap-1.5">
          {deck.subjectDomains.map((domain) => (
            <span
              key={domain}
              className="text-[10px] font-mono px-2.5 py-1 rounded-full bg-black/[0.04] dark:bg-white/[0.05] text-muted-foreground"
            >
              {domain}
            </span>
          ))}
