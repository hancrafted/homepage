...
export function Footer({ locale: explicitLocale }: { locale?: LocaleCode }) {
  ...
  return (
    <footer className="w-full border-t border-border/80 bg-muted/20 py-12 text-sm text-muted-foreground">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-8 border-b border-border/60">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-md">{SITE_CONTENT.footer.builtWith[locale]}</p>
          </div>
          <SocialLinks />
        </div>
